...
#python3 search_firing_sleep.py
...
furi = "DOWNM"  #chaging parameter to adjust mean firing rates
ISI_M=1.5   #log10(mean of Inter-spike interval) ISIM
UP_M = 2.7 #log10(mean of Up state duration)  UPM
DOWN_M= 1.8  #log10(mean of Down state duration) DOWNM
seed1= 30
...
#     print("UP_SD", UP_SD)
    ...
    return spikes, post_spike, spikes_ID, post_ID, mean_dur
...
#                     print("param", param)
                    if furi =="ISIM":
                        if param > ISI_lim:
                            break
                        spikes, post_spike,spikes_ID, post_ID, dur = gen_spikes_sleep(NE, (T+offset), dt, UP_th, DOWN_M, param, UP_M, seed1)

                    elif furi =="DOWNM":
                        if param <= DOWN_lim:
                            break
                        spikes, post_spike,spikes_ID, post_ID, dur = gen_spikes_sleep(NE, (T+offset), dt, UP_th, param, ISI_M, UP_M, seed1)
...
#                         rate_li.append(av_rate)
                        rate_li[fr_name_li[n]] = av_rate
                        rate_all_li[fr_name_li[n]] = av_rate
...
#                     print("param", param)
                    ...
                    spikes, post_spike,spikes_ID, post_ID, dur = gen_spikes_sleep(NE, (T+offset), dt, UP_th, DOWN_M, ISI_M, param, seed1)
                    av_rate = round(np.mean(np.sum(spikes, axis = 1)/((T+offset)/1000)), 4) #averaged rate
                    if count <3:
                        print("averaged rate", av_rate)
                    if av_rate >=fr - lim[n] and av_rate <fr + lim[n]:
