...
#                 spT = spts[spts.sum(axis=1) > 0., :]  #spikeあるものだけ選ぶ
    ...
    def sw_time(self,move,sw_cv, cv_th):
        ...
        for post_i in range(self.N):
            ...
            for tb in range(self.tbs):
                sp_file = self.drc_neu+ "ex"+str(self.ex)+"_"+"ex_in"+str(self.ex_in)+"_"+"Sp"+str(post_i)+"_"+str(tb)+".bin"
                fsp= open(sp_file, "rb")
                rectype = np.dtype(np.float64)
        ...
        for post_i in range(self.N):
            ...
            for tb in range(self.tbs):
                sp_file = self.drc_neu+ "ex"+str(self.ex)+"_"+"ex_in"+str(self.ex_in)+"_"+"K"+str(post_i)+"_"+str(tb)+".bin"
                fsp= open(sp_file, "rb")
                rectype = np.dtype(np.float64)
                sp_tb = np.fromfile(fsp, dtype=rectype).astype("float32")
                if tb==0:
    ...
    def rho_sw_cal(self, args):
        ...
        if len(sleep_durs)!=0:
            ...
            for n, dur_s in enumerate(sleep_durs):
                if dur_s[0]==0:
                    m_rho_s += np.mean(rho[dur_s[0]+int(200/self.dt):dur_s[1]])
                else:
                    m_rho_s += np.mean(rho[dur_s[0]:dur_s[1]])
        ...
        if len(wake_durs)!=0:
            ...
            for n,dur_w in enumerate(wake_durs):
                if dur_w[0]==0:
                    m_rho_w += np.mean(rho[dur_w[0]+int(200/self.dt):dur_w[1]])
                else:
                    m_rho_w += np.mean(rho[dur_w[0]:dur_w[1]])
