...
#define NO_IN_IN
// #define a_UP     //"_aup" -> update conductances by the second phosphorylated states of kinases, "_rup" -> update conductances by the initial phosphorylated states of kinases
...
//nvc++ -ta=tesla AN_network_dynamics_nmdar.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -Da_UP -lcudart -std=c++20 -o AN_net_aup_nmdar
//nvc++ -ta=tesla AN_network_dynamics_nmdar.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -Dr_UP -lcudart -std=c++20 -o AN_net_rup_nmdar
...
__global__ void dt_gpu_NaK1_n(double *v,  double *h_na, double *nk, double *h, double *m, double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3,
                    ...
                    double *Sp, double *Sp2,  double *K, double *K2,  double *z, double *z2, double *z3, double*r_no, double *rho, double *rho2, double *r, double max_con,double rho_ini, double smin_ampa, double smax_ampa, 
  ...
  for(int it=0; it<T; it++){
    ...
    {  
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double dKdt = ((1/(1+std::exp(-(Sp_n -beta*ca_syn -adp)*bdp))) - K_n)*dt/tau_k;
            // #endif
          
           
            double dSpdt = ((1/(1+std::exp(-(-b*K_n + I+z3[i] + w*Sp_n + alpha*ca_syn-adp2)*bdp2))) - Sp_n)*dt/tau_camk;
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double r_no_n  = r_no[i*T+it-1];
            double dKdt = ((1/(1+std::exp(-(Sp2[i]-beta*ca_syn -adp)*bdp))) - K2[i])*dt/tau_k;
            // #endif
            double dSpdt = (1/(1+std::exp(-(-b*K2[i] + I+z2[i] + w*Sp2[i] + alpha*ca_syn -adp2)*bdp2)) - Sp2[i])*dt/tau_camk;
...
int main(int argc, char * argv[]){
   ...
   //malloc host memory
    double *h_na, *nk, *h,*m, *ca, *s_a, *s_g, *x_n, *s_n, *z;
    ...
    double *d_x_n, *d_x_n2, *d_x_n3,*d_s_n, *d_s_n2, *d_s_n3,*d_Sp2,  *d_K2,  *d_z, *d_z2, *d_z3;
  ...
  double *v,  *Sp, *K, *cpre, *cpost, *rho, *r, *r_no;
    ...
    //  if (it != 0){
           ...
           CHECK(cudaMemcpy(d_rho, rho, b_syn_ex, cudaMemcpyHostToDevice));
            ...
            dt_gpu_NaK1_n<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, 
                         ...
                         d_Sp, d_Sp2,   d_K, d_K2,  d_z, d_z2, d_z3, d_r_no,  d_rho, d_rho2, d_r, max_con, rho_ini, smin_ampa, smax_ampa,
