...
//nvc++ -ta=tesla AN_network_stim_cav.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o AN_net_stim_cav
...
__global__ void dt_gpu_NaK1_n_stim(double *v,  double *h_na, double *nk, double *h, double *m, double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3,
                    ...
                    double *mp_pres,double *mp_posts, int stim_start, int stim_T, int *pre_stimed, int *post_stimed, int pre_stim_N, int post_stim_N, int delay)
{
  ...
  for(int it=0; it<T; it++){
    ...
    if (ix < N)  //ix neuron
    {  
        int stim = 0; //1:pre, 2:post, 0: no stim

        for(int sn=0; sn<pre_stim_N; sn++){
            if (ix == pre_stimed[sn]){  //&& ( stim_start <= it < stim_start+stim_T)
            stim=1;
            break; //for
            }
        }
        ...
        if (stim==0){
        for(int sn=0; sn<post_stim_N; sn++){
            if (ix == post_stimed[sn]){  //&& ( stim_start <= it < stim_start+stim_T)
            stim=2;
            break; //for
            }
         }
        ...
        double v_n;    //||(stim==2)&&(it < delay)
        if ((stim==0)){
            v_n =  v[ix*T + it-1];
        }else if(stim==1){
            v_n = mp_pres[T*tb+it];
        }else{
            v_n=mp_posts[T*tb+it];
        }
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double drho = drho_cal(rho_n, ca_syn, th_p,th_d, gp, gd, sig,  tau_s,  dt, r_n);
        ...
        if ((stim==0)){//||(stim==2)&&(it < delay)){
            v2 = v_n+0.5*dvdt;
           v[ix*T+it] = v_n+dvdt/6.0;
        }else{
     ...
     //update v
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double drho = drho_cal(rho2[i], ca_syn, th_p,th_d, gp, gd, sig,  tau_s,  dt, r_n);
...
__global__ void dt_gpu_NaK1_n(double *v,  double *h_na, double *nk, double *h, double *m, double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3,
                    double *x_n, double *x_n2, double *x_n3, double *s_n, double *s_n2, double *s_n3, double *cpre, double *cpre2,  double *cpost, double *cpost2, //double *S, double *S2, double *S3, 
                     double *rho, double *rho2, double *r,
                     int *neuron_syn_ex, int *synID_ex, int *syN_cumsum_ex, int *neuron_syn_in, int *synID_in, int *syN_cumsum_in, 
                     int N, int T,  double dt, double smin_ampa, double smax_ampa,
                    double g_leak, double g_nav,double g_kvhh,double g_kva, double g_kvsi, double g_cav,double g_cav_up, double g_kca, double g_nap, double g_kir, double g_ampar, double g_nmdar ,double g_gabar, double tau_ca,
                    double cpre_r, double cpost_r, double th_p, double th_d, double gp, double gd, double tau_ca_pre, double tau_ca_post, double sig, double tau_s
)
{
    unsigned int ix = threadIdx.x + blockIdx.x * blockDim.x;
...
int main(int argc, char * argv[]){
    ...
    double g_leak,g_nav,g_kvhh, g_kva, g_kvsi,g_cav_ori,g_nap,g_kca, g_kir, g_ampar, g_nmdar_ori, g_gabar, tau_ca;
    int g_c = 1;
    ...
    std::string taus_str = argv[g_c];
    g_c +=1;

    std::string lr = (argv[g_c]);
    g_c +=1;

     std::string lr_p = (argv[g_c]);
    g_c +=1;

    // int order = atoi(argv[g_c]);  //#ms
    // g_c +=1;
    ...
    int Gex = atoi(argv[g_c]);   //the number in a group of excitatory neurons
    g_c +=1;

    int *indexs =  (int*)malloc(sizeof(int)*(NE+NI));
    ...
    double con_M = atof(argv[g_c]);   //mean number of excitatry synapses
    std::string con_M_str = argv[g_c];
    g_c +=1;
    double con_M_in = atof(argv[g_c]);   //mean number of inhibitory synapses
    ...
    printf("dt %f\n", dt);
     ...
     int stim_start = atoi(argv[g_c]);
    std::string stim_start_str = argv[g_c];
    g_c +=1;  
   

    int stim_dur = atoi(argv[g_c]); //ms
      std::string stim_dur_str = argv[g_c];
    g_c +=1;

    printf("stim_dur %d\n", stim_dur);


    int hz = atoi(argv[g_c]); 
    std::string hz_str = argv[g_c];
    g_c +=1;

    double delay_ori = atof(argv[g_c]); 
    std::string delay_str = argv[g_c];
    g_c +=1;

    double peak = atoi(argv[g_c]); 
    std::string peak_str = argv[g_c];
    g_c +=1;

    double dur = atof(argv[g_c]); 
    std::string dur_str = argv[g_c];
    g_c +=1;
    double fy = atof(argv[g_c]); 
    std::string fy_str = argv[g_c];
    g_c +=1;
    double ly = atof(argv[g_c]); 
    std::string ly_str = argv[g_c];
    g_c +=1;
     double vrest0 = atof(argv[g_c]); 
    std::string vrest0_str = argv[g_c];
    g_c +=1;


     int sws_start = atoi(argv[g_c]);
    std::string sws_start_str = argv[g_c];
    g_c +=1;  
    int sws_dur = atoi(argv[g_c]); //ms
      std::string sws_dur_str = argv[g_c];
    g_c +=1;
    ...
    double ex_s = atof(argv[g_c]);
    std::string ex_s_str= argv[g_c];
    g_c +=1;


   
    double g_cav = g_cav_ori* (std::pow(10, ex));
    double g_nmdar = g_nmdar_ori;
   ...
   std::vector<std::vector<int>> neuron_cons(NE+NI, std::vector<int>(NE+NI));
    ...
    int* neuron_syN_in = (int*)malloc(sizeof(int)*(NE+NI));
          ...
          for (int i = 0; i < NE; i++) {   //post
             int con_num = (NE*con_ex_ex/100);
             
             for (int n = 0; n < con_num; n++) 
                { 
                    while(1){
          ...
          for (int i = 0; i < NE; i++) {  //post
             int con_num = (NI*con_in_ex/100);

                for (int n = 0; n < con_num; n++) 
                { 
                    while(1){
                double a = (double)rand()/RAND_MAX;
                int j = NE+ a*NI -0.001; //NE~NE+NI-1 random
                if ((i!=j) && (neuron_cons[i][j] != 1) ){
                        neuron_cons[i][j] = 1;
                        break;
                    }
        ...
        for (int i = NE; i < (NE+NI); i++) {  //post in
                 int con_num = (NE*con_ex_in/100);
             
             for (int n = 0; n < con_num; n++) 
                { 
                    while(1){
                double a = (double)rand()/RAND_MAX;
                int j = a*NE -0.001; //0~NE-1 random   pre ex
                if ((i!=j) && (neuron_cons[i][j] != 1) ){
                        neuron_cons[i][j] = 1;
                        break;
                    }
   ...
   int b_neuron2 = sizeof(double)*(NE+NI);
    ...
    int b_syn_i_ex = syN_sum_ex * sizeof(int);
     ...
     int *d_pre_stimed, *d_post_stimed;
     CHECK(cudaMalloc((void **)&d_pre_stimed, sizeof(int)*Gex));
     CHECK(cudaMalloc((void **)&d_post_stimed, sizeof(int)*Gex));
    

    //malloc host memory
    double *h_na, *nk, *h,*m, *ca, *s_a, *s_g, *x_n, *s_n;
    ...
    double *d_x_n, *d_x_n2, *d_x_n3,*d_s_n, *d_s_n2, *d_s_n3, *d_cpre2, *d_cpost2,*d_rho2;
    ...
    // CHECK(cudaMalloc((void **)&d_S, b_syn_ex2));
    // CHECK(cudaMalloc((void **)&d_S2, b_syn_ex2));
    // CHECK(cudaMalloc((void **)&d_S3, b_syn_ex2));
    ...
    #ifdef CON_LOGNORMAL
        std::string savedir = "./"+dir + "/NE"+std::to_string(NE)+"_"+ "NI"+std::to_string(NI)+"_Gex"+std::to_string(Gex) + "/conM"+ con_M_str +"_"+"conSD"+con_ex_ex_str +"_conMin"+ con_M_in_str +"_"+"conSDin"+con_in_in_str+"/sminampa"+smin_ampa_str+"_smaxampa"+smax_ampa_str+"/"+lr+"_"+lr_p+"/"+param_i+"/exs/"+ex_str+"/stim"+stim_start_str+"_"+stim_dur_str+"_"+hz_str+"hz"+"_delay"+delay_str+"_"+peak_str+"_"+dur_str+"_"+fy_str+"_"+ly_str+"_"+vrest0_str+"/sws"+sws_start_str+"_"+sws_dur_str+"_"+ex_s_str+"/seed_" +std::to_string(seed)+"/init_" +init+"/T_"+std::to_string(T)+"/Tp_"+std::to_string(Tp)+"/"; 
    #else
        std::string savedir = "./"+dir + "/NE"+std::to_string(NE)+"_"+ "NI"+std::to_string(NI) +"_Gex"+std::to_string(Gex)+ "/conpro"+con_ex_ex_str +"_"+con_ex_in_str +"_"++con_in_ex_str +"_"++con_in_in_str+"/sminampa"+smin_ampa_str+"_smaxampa"+smax_ampa_str+"/"+lr+"_"+lr_p+"/"+param_i+"/exs/"+ex_str+"/stim"+stim_start_str+"_"+stim_dur_str+"_"+hz_str+"hz"+"_delay"+delay_str+"_"+peak_str+"_"+dur_str+"_"+fy_str+"_"+ly_str+"_"+vrest0_str+"/sws"+sws_start_str+"_"+sws_dur_str+"_"+ex_s_str+"/seed_" +std::to_string(seed)+"/init_" +init+"/T_"+std::to_string(T)+"/Tp_"+std::to_string(Tp)+"/"; 
    ...
    r = (double*)malloc(b_syn_ex);
     ...
     int b_stim = sizeof(double)*stim_dur/dt;
    double *d_mp_pres, *d_mp_posts;
     std::vector<int> s;
    std::vector<double> mp_pres;
    std::string mp_pre_f = "./vstims/vpre_stim_"+hz_str+"hz_"+dt_str+ "_"+stim_dur_str +"_"+delay_str+"_"+peak_str+"_"+dur_str+"_"+fy_str+"_"+ly_str+"_"+vrest0_str+".npy";
    //  std::string mp_pre_f = "v_stim_40hz_0.05_5000.npy";
    std::cout << mp_pre_f<< std::endl;
    aoba::LoadArrayFromNumpy(mp_pre_f, s, mp_pres);

     std::vector<int> s1;
    std::vector<double> mp_posts;
    std::string mp_post_f = "./vstims/vpost_stim_"+hz_str+"hz_"+dt_str+ "_"+stim_dur_str +"_"+delay_str+"_"+peak_str+"_"+dur_str+"_"+fy_str+"_"+ly_str+"_"+vrest0_str+".npy";
    //  std::string mp_pre_f = "v_stim_40hz_0.05_5000.npy";
    std::cout << mp_post_f<< std::endl;
    aoba::LoadArrayFromNumpy(mp_post_f, s1, mp_posts);
    ...
    CHECK(cudaMalloc((void **)&d_mp_pres, b_stim));
    CHECK(cudaMemcpy(d_mp_pres, mp_pres.data(), b_stim, cudaMemcpyHostToDevice));
    CHECK(cudaMalloc((void **)&d_mp_posts, b_stim));
    CHECK(cudaMemcpy(d_mp_posts, mp_posts.data(), b_stim, cudaMemcpyHostToDevice));
    
    int stim_start_dt = stim_start/dt;
    int stim_dur_dt = stim_dur/dt;
    int delay = delay_ori/dt;
    ...
    int tb_start = stim_start/Tp;
    //  int tb_start2 = (stim_start+stim_dur+interval)/Tp;
    ...
    //  if (it != 0){
            ...
            if ((tb*Tp>=stim_start)&&(tb*Tp < stim_start+stim_dur)){
                std::vector <int> pre_stimed;
                for (int i=0; i<Gex; ++i) {
                    pre_stimed.push_back(i);
                }
            
                std::vector <int> post_stimed;
                for (int i=0; i<Gex; ++i) {
                    post_stimed.push_back(i+Gex);
                }
            
                int pre_stimed_size=pre_stimed.size();
                int post_stimed_size=post_stimed.size();
                

                CHECK(cudaMemcpy(d_pre_stimed, pre_stimed.data(), sizeof(int)*pre_stimed_size, cudaMemcpyHostToDevice));
                CHECK(cudaMemcpy(d_post_stimed, post_stimed.data(), sizeof(int)*post_stimed_size, cudaMemcpyHostToDevice));

                dt_gpu_NaK1_n_stim<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, 
                                ...
                                NE+NI, Ltp,  dt, smin_ampa, smax_ampa, tb-tb_start,
                                ...
                                d_mp_pres, d_mp_posts, stim_start_dt, stim_dur_dt, d_pre_stimed, d_post_stimed, pre_stimed_size,post_stimed_size, delay);
           
                }else if((tb*Tp>=stim_start+stim_dur + sws_start)&&(tb*Tp < stim_start+stim_dur + sws_start+sws_dur)){
                            
                    


                   
                    double g_cav_sws=g_cav_ori*std::pow(10, ex_s);
                    printf("gcav_sws %f\n", g_cav_sws);
                ...
                NE+NI, Ltp, dt, smin_ampa, smax_ampa,
                g_leak, g_nav, g_kvhh, g_kva, g_kvsi, g_cav, g_cav_sws,g_kca, g_nap,g_kir, g_ampar, g_nmdar, g_gabar,tau_ca,
                cpre_r, cpost_r, th_p, th_d, gp, gd,tau_ca_pre, tau_ca_post, sig, tau_s
                );
            ...
            dt_gpu_NaK1_n<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, 
                            ...
                            NE+NI, Ltp,  dt, smin_ampa, smax_ampa,
                            g_leak, g_nav, g_kvhh, g_kva, g_kvsi, g_cav,g_cav, g_kca, g_nap,g_kir, g_ampar, g_nmdar, g_gabar,tau_ca,
                            cpre_r, cpost_r, th_p, th_d, gp, gd,tau_ca_pre, tau_ca_post, sig, tau_s
                            );
              }
   ...
   CHECK(cudaFree(d_pre_stimed));
   CHECK(cudaFree(d_post_stimed));
