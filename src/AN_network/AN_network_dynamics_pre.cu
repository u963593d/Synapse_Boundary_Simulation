...
//nvc++ -ta=tesla AN_network_dynamics_pre.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o AN_net_aup_pre
//nvc++ -ta=tesla AN_network_dynamics_pre.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2022_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o AN_net_rup_pre
...
__global__ void dt_gpu_NaK1_n(double *v,  double *h_na, double *nk, double *h, double *m, double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3,
                    double *x_n, double *x_n2, double *x_n3, double *s_n, double *s_n2, double *s_n3, double *cpre, double *cpre2,  double *cpost, double *cpost2, double *preca_ex, double *preca_in,
                    double *Sp, double *Sp2,  double *Sp_in, double *Sp_in2,   double *K, double *K2,   double *K_in, double *K_in2, double *z, double *z2, double *z3,double *z_in, double *z_in2, double *z_in3, 
                    double*r_no,double*r_no_in,  double *rho, double *rho2, double *r, double max_con,double rho_ini, double smin_ampa, double smax_ampa, 
                    ...
                    double s_a_ampar, double x_a_nmdar, double s_a_gabar)
{
  ...
  for(int it=0; it<T; it++){
    ...
    {  
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double preca = preca_ex[synID_ex[i]];  //ca2+ in pre neuron
            


            double Sp_n = Sp[i*T+it-1];
            double K_n = K[i*T+it-1];
            // double z_n  = z[i*T+it-1]; 
            double r_no_n  = r_no[i*T+it-1];
            ...
            double dsdt_a = dsdt_a_cal(s_a_ampar * g_n, v[synID_ex[i]*T + it-1], s_a3[i], dt);
            // if (it==1){
            // printf("k1 ix %d, synID_ex[i]: %d\n", ix, synID_ex[i]);
            // }
            double dxdt_n = dxdt_n_cal(x_a_nmdar * g_n, v[synID_ex[i]*T + it-1], x_n3[i], dt);
            ...
            double dKdt = ((1/(1+std::exp(-(Sp_n -beta*preca -adp)*bdp))) - K_n)*dt/tau_k;

            double dSpdt = ((1/(1+std::exp(-(-b*K_n + I+z3[i] + w*Sp_n + alpha*preca-adp2)*bdp2))) - Sp_n)*dt/tau_camk;

            double dzdt = (-th * z3[i] + sig_n * std::sqrt(2*th/dt)*r_no_n)*dt;
            ...
            i_nmdar += i_nmda_c(g_nmdar, 1, s_n3[i], v_n);
            ...
            Sp2[i] =  Sp_n + 0.5*dSpdt;   //next s_a
            Sp[i*T+it] =  Sp_n + dSpdt/6.0;
            K2[i] =  K_n + 0.5*dKdt;   //next s_a
            K[i*T+it] = K_n + dKdt/6.0;
        ...
        for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in+syN_in); i++){
            ...
            double preca = preca_in[synID_in[i]];  //ca2+ in pre neuron
            double Sp_n = Sp_in[i*T+it-1];
            double K_n = K_in[i*T+it-1];
            // double z_n  = z[i*T+it-1]; 
            double r_no_n  = r_no_in[i*T+it-1];
            ...
            double dsdt_g = dsdt_g_cal(s_a_gabar*g_n, v[synID_in[i]*T + it-1], s_g3[i], dt);
            double dKdt = ((1/(1+std::exp(-(Sp_n -beta*preca -adp)*bdp))) - K_n)*dt/tau_k;
            double dSpdt = ((1/(1+std::exp(-(-b*K_n + I+z_in3[i] + w*Sp_n + alpha*preca-adp2)*bdp2))) - Sp_n)*dt/tau_camk;
            double dzdt = (-th * z_in3[i] + sig_n * std::sqrt(2*th/dt)*r_no_n)*dt;
            
            i_gabar += g_gabar * s_g3[i] * (v_n-vGABAR);

            // if ((ix==0)&&(i==0)&&(it<5)){
            // printf("k1_ini ix %d, i: %d,it:%d, preca: %f, i_gabar %f\n", ix, i, it, preca, i_gabar);
            // }
            ...
            z_in2[i] =  z_in3[i] + 0.5*dzdt;   //next s_a
            z_in[i] =  z_in3[i] + dzdt/6.0;
       
            Sp_in2[i] =  Sp_n + 0.5*dSpdt;   //next s_a
            Sp_in[i*T+it] =  Sp_n + dSpdt/6.0;
            K_in2[i] =  K_n + 0.5*dKdt;   //next s_a
            K_in[i*T+it] = K_n + dKdt/6.0;
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double preca = preca_ex[synID_ex[i]];


           
            
            #ifdef a_UP
                double g_n = K2[i]*max_con +G_MIN;
                if (g_n<1.0){
                    g_n=1.0;
                }
            #else
                double g_n = Sp2[i]*max_con +G_MIN;
                if (g_n<1.0){
                    g_n=1.0;
                }
            #endif
            // double dSdt, dSpdt;
            // double r_n = r[i*T+it-1];
            double dsdt_a = dsdt_a_cal(s_a_ampar*g_n, v[synID_ex[i]*T + it-1], s_a2[i], dt);
            // if (it==1){
            // printf("k1 ix %d, synID_ex[i]: %d\n", ix, synID_ex[i]);
            // }
            double dxdt_n = dxdt_n_cal(x_a_nmdar*g_n, v[synID_ex[i]*T + it-1], x_n2[i], dt);
            double dsdt_n = dsdt_n_cal(x_n2[i], s_n2[i], dt);

            #ifdef MG_BLOCK
                double i_nmdar_i = i_nmda_s(g_nmdar, 0.1, s_n2[i], v2) * mg_v(v2);
            #else
                #ifdef CA_SYN_CHANGE
            ...
            double r_no_n  = r_no[i*T+it-1];
            double dKdt = ((1/(1+std::exp(-(Sp2[i]-beta*preca -adp)*bdp))) - K2[i])*dt/tau_k;
            // #endif
            double dSpdt = (1/(1+std::exp(-(-b*K2[i] + I+z2[i] + w*Sp2[i] + alpha*preca -adp2)*bdp2)) - Sp2[i])*dt/tau_camk;
            double dzdt = (-th * z2[i] + sig_n * std::sqrt(2*th/dt)*r_no_n)*dt;
            ...
            Sp2[i] =  Sp[i*T+it-1] + 0.5*dSpdt;   //next s_a
            Sp[i*T+it] += dSpdt/3.0;
            K2[i] =  K[i*T+it-1] + 0.5*dKdt;   //next s_a
            K[i*T+it] += dKdt/3.0;
        ...
        i_gabar = 0; 
         for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in + syN_in); i++){
            ...
            #ifdef a_UP
                double g_n = K_in2[i]*max_con +G_MIN;
                if (g_n<1.0){
                    g_n=1.0;
                }
            #else
                double g_n = Sp_in2[i]*max_con +G_MIN;
                if (g_n<1.0){
                    g_n=1.0;
                }
            #endif
            ...
            double dsdt_g = dsdt_g_cal(s_a_gabar*g_n, v[synID_in[i]*T + it-1], s_g2[i], dt);

          
            double dKdt = ((1/(1+std::exp(-(Sp_in2[i]-beta*preca -adp)*bdp))) - K_in2[i])*dt/tau_k;
            // #endif
            double dSpdt = (1/(1+std::exp(-(-b*K_in2[i] + I+z_in2[i] + w*Sp_in2[i] + alpha*preca -adp2)*bdp2)) - Sp_in2[i])*dt/tau_camk;
            double dzdt = (-th * z_in2[i] + sig_n * std::sqrt(2*th/dt)*r_no_n)*dt;

            z_in2[i] =  z_in3[i] + 0.5*dzdt;   //next s_a
            z_in[i] += dzdt/3.0;
            Sp_in2[i] =  Sp_in[i*T+it-1] + 0.5*dSpdt;   //next s_a
            Sp_in[i*T+it] += dSpdt/3.0;
            K_in2[i] =  K_in[i*T+it-1] + 0.5*dKdt;   //next s_a
            K_in[i*T+it] += dKdt/3.0;
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            cpost[i*T+it]  += dcpost/3.0;   //update s_a
            Sp2[i] =  Sp[i*T+it-1] + dSpdt;   //next s_a
            Sp[i*T+it] += dSpdt/3.0;
            K2[i] =  K[i*T+it-1] + dKdt;   //next s_a
            K[i*T+it] += dKdt/3.0;
        ...
        i_gabar = 0; 
         for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in + syN_in); i++){
            ...
            z_in2[i] =  z_in3[i] + dzdt;   //next s_a
            z_in[i] += dzdt/3.0;
            Sp_in2[i] =  Sp_in[i*T+it-1] + dSpdt;   //next s_a
            Sp_in[i*T+it] += dSpdt/3.0;
            K_in2[i] =  K_in[i*T+it-1] + dKdt;   //next s_a
            K_in[i*T+it] += dKdt/3.0;
        ...
        i_gabar = 0; 
         for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in + syN_in); i++){
            ...
            Sp_in[i*T+it] += dSpdt/6.0;

            K_in[i*T+it] += dKdt/6.0;
    ...
    __syncthreads();
        ...
        //         break;
    for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
        preca_ex[synID_ex[i]] = -cpost_r * a_ca * 10*area*i_cav(g_cav, v[synID_ex[i]*T+it]);
    }

     for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in+syN_in); i++){
        preca_in[synID_in[i]] = -cpost_r * a_ca * 10*area*i_cav(g_cav, v[synID_in[i]*T+it]);
    }
...
int main(int argc, char * argv[]){
   ...
   double rho_ini    = atof(argv[g_c]);
    ...
    // int Lt = T/dt;
    std::cout << "dt_start" << dt_start  << std::endl;
   ...
   //malloc host memory
    ...
    double *d_x_n, *d_x_n2, *d_x_n3,*d_s_n, *d_s_n2, *d_s_n3,*d_Sp2,  *d_K2,  *d_z, *d_z2, *d_z3,*d_Sp_in2,  *d_K_in2,  *d_z_in, *d_z_in2, *d_z_in3;
    double  *d_preca_ex, *d_preca_in, *d_cpre2, *d_cpost2, *d_rho2;
    ...
    CHECK(cudaMalloc((void **)&d_z, b_syn_ex2));
    CHECK(cudaMalloc((void **)&d_z2, b_syn_ex2));
    CHECK(cudaMalloc((void **)&d_z3, b_syn_ex2));
    CHECK(cudaMalloc((void **)&d_z_in, b_syn_in2));
    CHECK(cudaMalloc((void **)&d_z_in2, b_syn_in2));
    CHECK(cudaMalloc((void **)&d_z_in3, b_syn_in2));
    
 

      CHECK(cudaMalloc((void **)&d_preca_ex, b_syn_ex2));
    CHECK(cudaMalloc((void **)&d_Sp2, b_syn_ex2));
    CHECK(cudaMalloc((void **)&d_K2, b_syn_ex2));
     CHECK(cudaMalloc((void **)&d_preca_in, b_syn_in2));
    CHECK(cudaMalloc((void **)&d_Sp_in2, b_syn_in2));
    CHECK(cudaMalloc((void **)&d_K_in2, b_syn_in2));
    // CHECK(cudaMalloc((void **)&d_dSp, b_syn_ex));
    // CHECK(cudaMalloc((void **)&d_dK, b_syn_ex));
    ...
    CHECK(cudaMalloc((void **)&d_rho2, b_syn_ex2));
  ...
  double *v, *Sp, *K, *Sp_in, *K_in, *cpre, *cpost, *rho, *r, *r_no,  *r_no_in ;
    ...
    Sp = (double*)malloc(b_syn_ex);
    K = (double*)malloc(b_syn_ex);
     Sp_in = (double*)malloc(b_syn_in);
    K_in = (double*)malloc(b_syn_in);
    // dSp = (double*)malloc(b_syn_ex);
    // dK = (double*)malloc(b_syn_ex);
    r_no = (double*)malloc(b_syn_ex);
    r_no_in = (double*)malloc(b_syn_in);

     rho = (double*)malloc(b_syn_ex);
    r = (double*)malloc(b_syn_ex);
...
//    printf("2\n");
    ...
    double *d_v;
    double *d_Sp,  *d_K,  *d_r_no, *d_Sp_in,  *d_K_in,  *d_r_no_in;
    ...
    CHECK(cudaMalloc((void **)&d_r_no, b_syn_ex));
    CHECK(cudaMalloc((void **)&d_Sp, b_syn_ex));
    CHECK(cudaMalloc((void **)&d_K, b_syn_ex));
    CHECK(cudaMalloc((void **)&d_r_no_in, b_syn_in));
    CHECK(cudaMalloc((void **)&d_Sp_in, b_syn_in));
    CHECK(cudaMalloc((void **)&d_K_in, b_syn_in));
    ...
    CHECK(cudaMalloc((void **)&d_rho, b_syn_ex));
    CHECK(cudaMalloc((void **)&d_r, b_syn_ex));
    ...
    CHECK(cudaMemcpy(d_ca, ca, b_neuron2, cudaMemcpyHostToDevice));
     CHECK(cudaMemcpy(d_preca_ex, preca_ex, b_syn_ex2, cudaMemcpyHostToDevice));
     CHECK(cudaMemcpy(d_preca_in, preca_in, b_syn_in2, cudaMemcpyHostToDevice));
     ...
     CHECK(cudaMemcpy(d_z, z, b_syn_ex2, cudaMemcpyHostToDevice));
     CHECK(cudaMemcpy(d_z_in, z_in, b_syn_in2, cudaMemcpyHostToDevice));
    ...
    //  if (it != 0){
            ...
            for (int i=0; i<syN_sum_in; ++i) {
                ...
                // printf("number1 %f\n",number);
            #ifdef RANDOM
                r_no_in[i*Ltp + it] = number;
            #else
                r_no_in[i*Ltp + it] =constant_num; 
                // printf("%d\n",i*Ltp+it);
            #endif
            ...
            CHECK(cudaMemcpy(d_r_no, r_no, b_syn_ex, cudaMemcpyHostToDevice));
             CHECK(cudaMemcpy(d_r_no_in, r_no_in, b_syn_in, cudaMemcpyHostToDevice));
            ...
            CHECK(cudaMemcpy(d_Sp, Sp, b_syn_ex, cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(d_K, K, b_syn_ex, cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(d_Sp_in, Sp_in, b_syn_in, cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(d_K_in, K_in, b_syn_in, cudaMemcpyHostToDevice));
            // CHECK(cudaMemcpy(d_dSp, dSp, b_syn_ex, cudaMemcpyHostToDevice));
            // CHECK(cudaMemcpy(d_dK, dK, b_syn_ex, cudaMemcpyHostToDevice));
           CHECK(cudaMemcpy(d_rho, rho, b_syn_ex, cudaMemcpyHostToDevice));
            CHECK(cudaMemcpy(d_r, r, b_syn_ex, cudaMemcpyHostToDevice));
            ...
            dt_gpu_NaK1_n<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, 
                            d_x_n, d_x_n2, d_x_n3, d_s_n, d_s_n2, d_s_n3, d_cpre, d_cpre2, d_cpost, d_cpost2, d_preca_ex, d_preca_in, 
                         d_Sp, d_Sp2, d_Sp_in, d_Sp_in2,  d_K, d_K2,d_K_in, d_K_in2,  d_z, d_z2, d_z3, d_z_in, d_z_in2, d_z_in3,d_r_no, d_r_no_in, d_rho, d_rho2, d_r, max_con, rho_ini, smin_ampa, smax_ampa,
           ...
           // check kernel error
            ...
            CHECK(cudaMemcpy(Sp, d_Sp, b_syn_ex, cudaMemcpyDeviceToHost));
            CHECK(cudaMemcpy(K, d_K, b_syn_ex, cudaMemcpyDeviceToHost));
             CHECK(cudaMemcpy(Sp_in, d_Sp_in, b_syn_in, cudaMemcpyDeviceToHost));
            CHECK(cudaMemcpy(K_in, d_K_in, b_syn_in, cudaMemcpyDeviceToHost));
            // CHECK(cudaMemcpy(dSp, d_dSp, b_syn_ex, cudaMemcpyDeviceToHost));
            // CHECK(cudaMemcpy(dK, d_dK, b_syn_ex, cudaMemcpyDeviceToHost));
            CHECK(cudaMemcpy(rho, d_rho, b_syn_ex, cudaMemcpyDeviceToHost));
            //計算したvを保存用配列に代入
        ...
        for (int i=0; i<NE+NI; i++){
            ...
            for (int s=syN_cumsum_ex_i; s<syN_cumsum_ex_i+syN_ex; s++){
                ...
                std::string save_Sp = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"Sp"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                std::string save_K = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"K"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                
                
                // std::string save_dSp = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"dSp"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                // std::string save_dK = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"dK"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                ...
                for (int it=0; it<Ltp-1; it++){
                    ofs_Sp.write(( char * ) &Sp[s*Ltp+it],sizeof( double ) );
                    }//for
                ofs_Sp.close();
                ...
                for (int it=0; it<Ltp-1; it++){
                    ofs_K.write(( char * ) &K[s*Ltp+it],sizeof( double ) );
                    }//for
                ofs_K.close();

                // ofs_K.open(save_dK, std::ios::out|std::ios::binary|std::ios::trunc);
                // if (!ofs_K) {
                // std::cout << "Can't open a file"<<save_K<<std::endl;
                // }
                // // std::cout <<save_K<<std::endl;
                // for (int it=0; it<Ltp-1; it++){
                //     ofs_K.write(( char * ) &dK[s*Ltp+it],sizeof( double ) );
                //     }//for
                // ofs_K.close();
            ...
            //save inhibitory
            int syN_in = neuron_syN_in[i];
            int syN_cumsum_in_i = syN_cumsum_in[i];
            for (int s=syN_cumsum_in_i; s<syN_cumsum_in_i+syN_in; s++){
                int pre_synID = synID_in[s];
                std::string save_Sp_in = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"Spin"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                std::string save_K_in = savedir_syn+"ex"+ex_str +"_"+"ex_in"+ex_in_str+"_"+"Kin"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                
                 std::ofstream ofs_Sp;
                ofs_Sp.open(save_Sp_in, std::ios::out|std::ios::binary|std::ios::trunc);
                if (!ofs_Sp) {
                std::cout << "Can't open a file"<<save_Sp_in<<std::endl;
                }
                // std::cout <<save_Sp<<std::endl;
                for (int it=0; it<Ltp-1; it++){
                    ofs_Sp.write(( char * ) &Sp_in[s*Ltp+it],sizeof( double ) );
                    }//for
                ofs_Sp.close();
        

                std::ofstream ofs_K;
                ofs_K.open(save_K_in, std::ios::out|std::ios::binary|std::ios::trunc);
                if (!ofs_K) {
                std::cout << "Can't open a file"<<save_K_in<<std::endl;
                }
                // std::cout <<save_K<<std::endl;
                for (int it=0; it<Ltp-1; it++){
                    ofs_K.write(( char * ) &K_in[s*Ltp+it],sizeof( double ) );
                    }//for
                ofs_K.close();
        ...
        for (int i = 0; i < (NE+NI); i++) {
         ...
         for (int i = 0; i < syN_sum_in; i++) {
            Sp_in[i*Ltp] = Sp_in[i*Ltp + Ltp-1];
            K_in[i*Ltp] = K_in[i*Ltp + Ltp-1];
         }
    ...
    CHECK(cudaFree(d_K));
    CHECK(cudaFree(d_r_no_in));
   
    CHECK(cudaFree(d_Sp_in));
 
    CHECK(cudaFree(d_K_in));
    ...
    CHECK(cudaFree(d_ca));
     CHECK(cudaFree(d_preca_ex));
     CHECK(cudaFree(d_preca_in));
    ...
    CHECK(cudaFree(d_z2));
    CHECK(cudaFree(d_z3));
    CHECK(cudaFree(d_z_in));
    CHECK(cudaFree(d_z_in2));
    CHECK(cudaFree(d_z_in3));
  
 
    CHECK(cudaFree(d_Sp2));
   
    CHECK(cudaFree(d_K2));
    CHECK(cudaFree(d_Sp_in2));
   
    CHECK(cudaFree(d_K_in2));
