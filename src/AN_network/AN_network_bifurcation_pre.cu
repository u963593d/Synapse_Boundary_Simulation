...
__global__ void dt_gpu_NaK1_n(double *v,  double *h_na, double *nk, double *h, double *m,double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3, double *x_n, double *x_n2, double *x_n3, double *s_n, double *s_n2, double *s_n3, int *neuron_syn_ex, int *synID_ex, int *syN_cumsum_ex, int *neuron_syn_in, int *synID_in, int *syN_cumsum_in, int N, int T, double dt,  
                                             double g_leak, double g_nav,double g_kvhh,double g_kva, double g_kvsi, double g_cav,double g_kca, double g_nap, double g_kir, double g_ampar, double g_nmdar ,double g_gabar, double tau_ca, double s_a_ampar, double x_a_nmdar, double s_a_nmdar,  double s_a_gabar)
{
   //v_nw: updated v
    ...
    {  
       ...
       i_nmdar = 0;
         for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex + syN_ex); i++){
            ...
            s_n2[i] =  s_n3[i] + 0.5*dsdt_n;   //next s_a
            s_n[i] +=  dsdt_n/3.0;
            // printf("k2 ix %d, synID_ex: %d, i_ampar %f\n", ix, synID_ex[i], i_ampar);
        }
        i_gabar = 0; 
         for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in + syN_in); i++){
            // printf("k2 ix %d, i: %d, v2 %f\n", ix, i, v2);
...
int main(int argc, char * argv[]){
    ...
    int nmdar = 1;
    int gabar = 1;
    double s_a_ampar_ori= 3.48;
    double s_a_nmdar_ori= 5 ;
    double x_a_nmdar_ori= 34.8;
    double s_a_gabar_ori =1;
  ...
  for (int k=0; k<ex_v.size(); k++){  //ex parallel start
    ...
    double s_a_ampar = s_a_ampar_ori *  std::pow(10, ex);
    double s_a_nmdar = s_a_nmdar_ori ;
    double x_a_nmdar = x_a_nmdar_ori *  std::pow(10, ex);
    double s_a_gabar = s_a_gabar_ori *  std::pow(10, ex);
    ...
    //  if (it != 0){
            ...
            dt_gpu_NaK1_n<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, d_x_n, d_x_n2, d_x_n3, d_s_n, d_s_n2, d_s_n3, d_neuron_syN_ex, d_synID_ex, d_syN_cumsum_ex, d_neuron_syN_in, d_synID_in, d_syN_cumsum_in, 
                                        NE+NI, Ltp,  dt, g_leak, g_nav, g_kvhh, g_kva, g_kvsi, g_cav,g_kca, g_nap,g_kir, g_ampar, g_nmdar, g_gabar,tau_ca,  s_a_ampar,   x_a_nmdar, s_a_nmdar,  s_a_gabar);
            }
