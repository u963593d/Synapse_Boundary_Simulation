...
//nvc++ -ta=tesla AN_network_efficacy_pre.cu -I/opt/nvidia/hpc_sdk/Linux_x86_64/23.3/examples/OpenACC/SDK/include/ -I/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/include -L/tmp/nvhpc_2023_233_Linux_x86_64_cuda_multi/install_components/Linux_x86_64/23.3/math_libs/12.0/targets/x86_64-linux/lib -lcudart -std=c++20 -o AN_net_effi_pre
...
__device__ double dsdt_a_cal(double v, double s_a, double dt, double s_a_ampar){
...
__device__ double dxdt_n_cal(double v, double x_n, double dt,  double x_a_nmdar){
        // double fv = 1.0 / (1.0 + std::exp(-(v-20)/2.0));
...
__device__ double dsdt_n_cal(double x_n, double s_n, double dt,  double s_a_nmdar){
        // double fv = 1.0 / (1.0 + std::exp(-(v-20)/2.0));
        double dsdt_n = (s_a_nmdar * x_n * (1-s_n) - s_n/s_tau_nmdar)*dt;
     return dsdt_n;
}

__device__ double dsdt_g_cal(double v, double s_g, double dt,  double s_a_gabar){
        // double fv = 1.0 / (1.0 + std::exp(-(v-20)/2.0));
...
__global__ void dt_gpu_NaK1_n(double *v,  double *h_na, double *nk, double *h, double *m, double *ca, double *s_a, double *s_a2, double *s_a3, double *s_g, double *s_g2, double *s_g3,
                    ...
                    double cpre_r, double cpost_r, double th_p, double th_d, double gp, double gd, double tau_ca_pre, double tau_ca_post, double sig, double tau_s, double s_a_ampar, double x_a_nmdar, double s_a_nmdar,  double s_a_gabar)
{
  ...
  for(int it=0; it<T; it++){
    ...
    {  
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double dsdt_a = dsdt_a_cal(v[synID_ex[i]*T + it-1], s_a3[i], dt,  s_a_ampar);
            double dxdt_n = dxdt_n_cal(v[synID_ex[i]*T + it-1], x_n3[i], dt, x_a_nmdar);
            double dsdt_n = dsdt_n_cal(x_n3[i], s_n3[i], dt, s_a_nmdar);
        ...
        for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in+syN_in); i++){
            s_g3[i] = s_g[i];
            // printf("k1 ix %d, i: %d, v_n %f\n", ix, i, v_n);
            double dsdt_g = dsdt_g_cal(v[synID_in[i]*T + it-1], s_g3[i], dt, s_a_gabar);
            // if (it==1){
        ...
        for (int i= syN_cumsum_i_ex; i<(syN_cumsum_i_ex+syN_ex); i++){
            ...
            double dsdt_a = dsdt_a_cal(v[synID_ex[i]*T+it-1], s_a2[i], dt, s_a_ampar);
            double dxdt_n = dxdt_n_cal(v[synID_ex[i]*T + it-1], x_n2[i], dt, x_a_nmdar);
            double dsdt_n = dsdt_n_cal(x_n2[i], s_n2[i], dt, s_a_nmdar);
        ...
        i_gabar = 0; 
         for (int i= syN_cumsum_i_in; i<(syN_cumsum_i_in + syN_in); i++){
            ...
            double dsdt_g = dsdt_g_cal(v[synID_in[i]*T+it-1], s_g2[i], dt, s_a_gabar);
...
int main(int argc, char * argv[]){
 ...
 //   #if defined(NMDAR)
   ...
   // double s_a_nmdar = s_a_nmdar ;
    x_a_nmdar = x_a_nmdar *  std::pow(10, ex);
    s_a_gabar = s_a_gabar *  std::pow(10, ex);
   
    
    // #ifdef GABAUP 
    //     g_gabar = g_gabar * (std::pow(10, ex+gabac)) ;
    // #else
    //     g_gabar = g_gabar * gabac;
    // #endif
   ...
   int b_neuron2 = sizeof(double)*(NE+NI);
    ...
    #ifdef CON_LOGNORMAL
        std::string savedir = "./"+dir + "/NE"+std::to_string(NE)+"_"+ "NI"+std::to_string(NI) + "/conM"+ con_M_str +"_"+"conSD"+con_ex_ex_str +"_conMin"+ con_M_in_str +"_"+"conSDin"+con_in_in_str+"/sminampa"+smin_ampa_str+"_smaxampa"+smax_ampa_str+"/"+lr+"_"+lr_p+"/"+param_i+"/exs/"+ex_str+"/seed_" +std::to_string(seed)+"/init_" +init+"/T_"+std::to_string(T)+"/Tp_"+std::to_string(Tp)+"/"; 
    #else
        std::string savedir = "./"+dir+"param_"+param_i+"/cprer"+cpre_r_str+"_cpostr"+cpost_r_str+"/rhoini"+rho_ini_str +"/NE"+std::to_string(NE)+"_"+ "NI"+std::to_string(NI) + "/con_th"+con_ex_ex_str+"_"+con_ex_in_str +"_"+con_in_ex_str+"_"+con_in_in_str +"/seed_" +std::to_string(seed)+"/init_" +init+"/T_"+std::to_string(T)+"/Tp_"+std::to_string(Tp)+"/"+lr+"_"+lr_p+"/"; 
    #endif
    ...
    //  if (it != 0){
            ...
            dt_gpu_NaK1_n<<<grid, block>>>(d_v, d_h_na, d_nk, d_h, d_m, d_ca, d_s_a, d_s_a2, d_s_a3,d_s_g, d_s_g2, d_s_g3, 
                            ...
                            s_a_ampar,   x_a_nmdar, s_a_nmdar,  s_a_gabar);
        ...
        for (int i=0; i<NE+NI; i++){
            std::string savef = savedir_neu+"N"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
            ...
            for (int s=syN_cumsum_ex_i; s<syN_cumsum_ex_i+syN_ex; s++){
                ...
                std::string save_cpre = savedir_syn+"cpre"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
                std::string save_cpost = savedir_syn+"cpost"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
               std::string save_rho = savedir_syn+"rho"+std::to_string(pre_synID)+"-"+std::to_string(i)+"_"+std::to_string(tb)+".bin";
